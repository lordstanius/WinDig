...
samp *buffer = NULL;
ushort firsts, last, size;           /* data available to output device */

int rate;
ushort t0rate, t2rate, t2new, t0v = 0, t2v = 0;
short i8pulse = 0;
...
void soundinitglob(int port, int irq, int dma, ushort bufsize, ushort samprate)
{
	...
	rate = (int)(0x1234ddul / (uint)samprate);
	firsts = 0;
	last = 1;
	size = bufsize << 1;
...
bool subcarry(ushort *dest, ushort sub)
{
	*dest -= sub;
	if (*dest >= (ushort)(-sub))
		return TRUE;
...
{
	bool f = FALSE, t2sw0;
	ushort spkrt2, noi8, complicate = 0, not2;
